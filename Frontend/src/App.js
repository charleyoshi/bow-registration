...
function App() {
  ...
  const enroll = (studentID, course) => {
    ...
    var maximum = 5;
    let alreadyEnrolledIn = enrollments.filter(
      (u) => u.studentID === studentID && u.term === course.term
    ); //[]
    if (isCertStudent) {
    ...
    if (alreadyEnrolledIn.length >= maximum) {
      return {
        status: "fail",
        message:
          "You have enrolled maximum number of courses. If you want to register for this course, you must drop other courses first.",
      };
    }
  ...
  const drop = (studentID, course) => {
    ...
    const filteredArray = enrollments.filter(
      (e) => e.studentID !== studentID || e.courseCode !== course.courseCode
    );
    ...
    if (alreadyEnrolledIn.length <= minimum) {
      return {
        status: "warning",
        message:
          "Warning: You are currently under-credit. Please make sure you take enough credits for graduation requirement.",
      };
    }
    return {
      status: "success",
      message: "You have dropped the course successfully.",
    };
  };
  ...
  const hasEnrolled = (studentID, course) => {
    ...
    const alreadyExist = enrollments.some(
      (item) =>
        Object.entries(item).toString() ===
        Object.entries(newEnrollment).toString()
    );
    if (alreadyExist) {
      return true;
    }
  ...
  const sendEnquiry = async (inquiry) => {
    // try {
